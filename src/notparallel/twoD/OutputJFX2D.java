	...
	public void initializeRectangle() {
		for (int ix = 0; ix < InitializeParameter.QB; ix++) {
			...
			for (int iy = 0; iy < InitializeParameter.QL; iy++) {
				...
				} else if (ix == 0 || ix == InitializeParameter.QB - 1
						|| leftSide && iy == InitializeParameter.QB - 1) {
				...
				for (int n = 1; n <= InitializeParameter.N; n++) {
					for (int iy = 1; iy < InitializeParameter.QL - 1; iy++) {
						for (int ix = 1; ix < InitializeParameter.QB - 1; ix++) {
							u2_2D[ix][iy] = u1_2D[ix][iy]
									+ (0.1f * (u1_2D[ix + 1][iy]
											+ u1_2D[ix - 1][iy] - (2 * u1_2D[ix][iy])))
									+ (0.1f * (u1_2D[ix][iy + 1]
											+ u1_2D[ix][iy - 1] - (2 * u1_2D[ix][iy])));
						}
					}
					...
					for (int iy = 0; iy < InitializeParameter.QL; iy++) {
						for (int ix = 0; ix < InitializeParameter.QB; ix++) {
							...
							if (rectangle != null) {
								rectangle
										.setFill(computeColor((float) u2_2D[ix][iy]));
							}
							// Neue Werte, sind jetzt alte Werte
							// Tausch InitializeParameter_3D.u1_2D =
							// InitializeParameter_3D.u2_2D, geht so nicht, da
							// arrayreferenzen
							// gebildet werden
